...
class _BaseSettings:
    ...
    def _process_doc(self, value):
        if isinstance(value, Path) or (
            os.path.isfile(value) and value.strip() == value
        ):
            try:
    ...
    def _split_log_level(self, level):
        ...
        if collect.priority > show.priority:
            self._raise_invalid(
                "LogLevel",
                f"Level in log '{show.level}' is lower than execution "
                f"level '{collect.level}'.",
            )
    ...
    def _process_max_error_lines(self, value):
        ...
        if value < 10:
            self._raise_invalid(
                "MaxErrorLines", f"Expected integer greater than 10, got {value}."
            )
        return value
    ...
    def _process_tag_stat_link(self, value):
        ...
        self._raise_invalid(
            "TagStatLink", f"Expected format 'tag:link:title', got '{value}'."
        )
...
class RobotSettings(_BaseSettings):
    ...
    def _names_and_rerun(self, for_test=False):
        if for_test:
            ...
            rerun = gather_failed_tests(
                self["ReRunFailed"],
                self["RunEmptySuite"],
            )
        else:
            names = self["SuiteNames"]
            rerun = gather_failed_suites(
                self["ReRunFailedSuites"],
                self["RunEmptySuite"],
            )
